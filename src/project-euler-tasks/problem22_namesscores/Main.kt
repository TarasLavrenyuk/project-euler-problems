...
object Main {
  ...
  fun main(args: Array<String>) {
    var result = 0
    readListOfNames().sorted().forEachIndexed { index, name ->
      result += (index + 1) * calculateNameWeight(name)
    }
    println(result)
  }

  private fun readListOfNames() = File("src/problem22_namesscores/names.txt").readText().replace("\"", "").split(",")

  private fun calculateNameWeight(name: String): Int {
    var result = 0
    name.forEach {
      result += it.toInt() - 64
    }
